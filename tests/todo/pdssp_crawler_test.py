...
def test_name():
    name = pds_crawler.__name_soft__
    assert name == "pds_crawler"
...
def test_new_level():
    pds_crawler.custom_logging.UtilsLogs.add_logging_level("TRACE", 20)
...
def test_message():
    record = pds_crawler.custom_logging.LogRecord(
        "__main__",
        logging.INFO,
        "pathname",
        2,
...
def test_color_formatter():
    formatter = pds_crawler.custom_logging.CustomColorFormatter()
    logger = logging.getLogger("pds_crawler.custom_logging")
    ...
    shell_formatter = pds_crawler.custom_logging.ShellColorFormatter()
    record = pds_crawler.custom_logging.LogRecord(
        "__main__",
        logging.INFO,
        "pathname",
